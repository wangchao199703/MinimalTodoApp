...
## v2.0.0 — 修:更新后旧进程残留(旧版设置弹窗不关、盖在新版上)

- **关键线索(用户)**:更新后旧版的「设置弹窗」没关闭——即**旧进程没退干净**,旧前端(无快捷键)残留盖在新版上,看着像「新版没生效」。
- **根因**:重装是从**设置弹窗**里点的,旧进程 `app.exit(0)` 优雅退出会被设置弹窗的 webview / 在途 IPC 卡住,导致旧进程及其窗口残留。
- **修复**:旧进程退出改为「优雅 + 兜底硬退」:先 `app.exit(0)`,1.2s 后无论是否退干净都 `std::process::exit(0)` **硬退**,确保旧进程及其所有窗口(含设置弹窗)立即消失。SQLite WAL 下硬退不丢不损。
- **注意(取舍)**:此修复在**旧版退出侧**,故仅当「从带此修复的版本再更新」时才生效;**本次这一跳**(从旧构建 → 本构建)仍可能残留旧窗口,手动关掉旧设置弹窗 / 退出重开即可,之后的更新不再残留。
- 验证:`cargo check` 无警告。版本保持 2.0.0。

## v2.0.0 — 修:打开新 exe 不接管旧实例(旧文件删不掉、看到的还是旧版)

- **现象**:旧版 `app(13)` 在跑,双击新下载的 `app(14)` → 单实例把 `app(14)` 挡回并聚焦 `app(13)`,于是 `app(13)` 仍在跑、其 exe 被占用删不掉,看到的还是旧版。手动重装同理。
- **根因**:单实例「后启动的让位给先启动的」是默认行为;新 exe 没接管旧实例。
- **修复(对齐 WPF「新版接管旧版」,且不依赖旧实例配合)**:启动时(注册单实例插件之前、打开数据库之前)用 **Windows Restart Manager** 找出**正占用 `todo.db` 的进程**(任何在跑的实例都锁着库,与 exe 名/版本/是否留标记无关)→ 强杀 → 本(新)实例接管。旧进程一退,旧 exe 文件随即解除占用、可删,新版成为活动实例。
...
- 三处版本号 2.0.0 → 2.0.1(tauri.conf.json / Cargo.toml / package.json)。
...
### 11) 剪切板工具栏/筛选面板再优化(参考第二份 Gemini 稿)(版本仍 2.0.1)
- 同样未照抄 shadcn token,映射到项目 token 落地。
- **筛选面板默认展开**(`filtersOpen` 初始 true),保留漏斗按钮可收起。
- **类型筛选**改与「大小」同款分段控制器(浮起式激活)。
- **日期范围**改触发按钮 + Popover:快捷范围(今天/近7天/近1个月/近3个月)+ 自定义起止 + 清除日期;按钮显示当前范围或占位。
- **去重开关内联**:筛选面板加 Apple 式 Switch「重复项置顶」,与设置同一持久化键 `clip_dedup`(两处联动)。
- 各模块用竖线分隔,「清除筛选」推到最右(hover 红);漏斗激活态改浅主题色填充。
- tsc 通过、HMR 无报错。

### 12) 剪切板:点击预览 + 默认分组过期 + 另存为(版本仍 2.0.1)
- **图片预览改单击触发**:去掉缩略图双击,改为**单击缩略图**即开灯箱(右键菜单「预览」保留)。
- **过期设置改名「默认分组过期时间」+ 只对默认分组**:`clip_purge_expired` SQL 加 `id NOT IN (SELECT clip_id FROM clip_tags)`,只清理未分组(默认分组)且未置顶的过期项;已分组 / 置顶项永久保留。i18n 标签与说明同步改。
- **右键另存为**:文本项「另存为文本」→ 桌面 `剪贴文本-时间戳.txt`(复用 export_file);图片项「保存图片到本地」→ 桌面 `剪贴图片-时间戳.png`(新增 `save_clip_image` 命令拷贝库内 PNG)。带时间戳避免覆盖,toast 回显完整路径。
- tsc + cargo check 通过。

### 13) 剪切板:移除大中小 → 浏览模式(相册式)(版本仍 2.0.1)
- **移除工具栏 S/M/L 大小切换**,列表固定中号;清空 `clip_item_size` 相关 UI(设置键保留无害)。
- **新增「浏览模式」**(工具栏 Images 按钮,持久化 `clip_browse_mode`):进入后右侧区只展示**当前一条**剪贴项,图片 `object-contain` 铺满、文本大字铺满(超长可滚)。
- **滚轮 / ↑↓←→ 方向键翻看**(相册式,160ms 节流),**Esc 退出**;上/下浮层翻页按钮 + 底部「i / N」计数与操作提示。
- 浏览的是当前**已筛选**集合(类型=图片 + 浏览 = 看图相册);列表变化自动夹紧索引。
- 滚轮用原生非被动监听 preventDefault,拦住页面滚动;tsc 通过、HMR 无报错。

### 14) 浏览模式:左右翻看 + 右键菜单(版本仍 2.0.1)
...
## v2.0.2 — 版本号提升 + 发布(便签编辑器升级)
- 三处版本号 2.0.1 → 2.0.2(tauri.conf.json / Cargo.toml / package.json)。
- `release-notes.md` 顶部新增 v2.0.2 段:汇总自**首个 v2.0.1 发布之后、尚未进入已发布 v2.0.1 说明的新变更**——即便签编辑器对标 Obsidian/Notion 的能力补齐(全局搜索 / 插入链接 / 撤销重做 / 引用代码块 / 大纲 / 斜杠命令 / 字数与修改时间 / 回收站 / 导出 HTML)、「显示为源码」就地可编辑 + 光标精确保留居中 + 源码态保留大纲、所有弹窗文案改自动换行。(条目 1–22 已包含在已发布的 v2.0.1 说明里,故不重复。)
- 发布动作:同一份 2.0.2 exe 产物推送到三个 release——**v2.0.2**(新建 release + 说明 + 资产),**v2.0.1 / v2.0.0**(仅替换资产,release 说明不动)。资产名沿用各自 tag 命名(`MinimalTodoApp-vX-win-x64.exe`)。

### v2.0.2 微调:便签回收站改为「分组式入口」(重发 v2.0.2)
- 需求:把回收站从工具栏的一个按钮,改成侧栏分组列表底部的一个「回收站」分组行(和普通分组同款入口样式),点击进入回收站视图。已与用户确认是「侧栏一个分组项」(非每分组各自回收站、非按原分组归类)。
- 改动:
  - `NotesTree.tsx`:分组列表底部新增 `TrashRow`(Trash2 图标 + 「回收站」+ 顶部细分隔线;`notesTrashOpen` 时高亮),点击 `setNotesTrashOpen` 切换。
  - `NotesView.tsx`:移除工具栏的回收站 Trash2 按钮(及未用的 import)。
...
## v2.0.4 — 发版交付剪贴板图片预览修复(版本必须递增)
- 背景:上一步把图片预览修复**重发到了同一个 v2.0.3**。但应用内自动更新按 SemVer 比对(`updater.ts`:`!newer(remote,current)` 即「已是最新」),装着 2.0.3 的用户看到最新仍是 2.0.3 → **不会提示更新**,修复无法经自动更新送达。且 v2.0.3 的 release 说明也没提这条修复。
- 解决:版本号 2.0.3 → 2.0.4(三处),`release-notes.md` 顶部加 v2.0.4 段(把「剪贴板图片预览修复」写进去,并保留四象限输入说明,覆盖从 2.0.2/2.0.3 升上来的用户)。代码与 v2.0.3(f9c0e48)一致,只是版本号递增使自动更新能识别。
- 发布:新建 v2.0.4 release(说明 + 资产),按惯例只发当前版本。
