...
**主题**(`src/lib/themes.ts` + `themes-builtin.json`):102 套内置(27 xaml 成品 + 72 锚点派生 + 3 磨砂)+ 用户自定义(存库)。应用方式 = 18 个颜色键写入 documentElement 的 CSS 变量(`index.css` 中 `@theme inline` 映射成 Tailwind 语义色,如 `bg-card`/`text-text-1`)。派生算法与旧版 `BuiltinThemes.Build` 逐像素一致。透明系分组自动启用原生亚克力(`window.rs::set_acrylic`)。
...
- 新增主题颜色键需同步:`COLOR_KEYS`、`VAR_NAME` 映射、`index.css` 的 `:root` 与 `@theme inline`、主题编辑器 `KEY_LABEL`。
