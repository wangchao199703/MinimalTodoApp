...
**主题**(`src/lib/themes.ts` + `index.css`,对齐 todo-flow):6 套 —— lumina / light / dark / warm / glass / system。light 为 `:root` 基线,其余是根节点 class(`.dark`/`.glass`/`.warm`/`.lumina`),system 由 matchMedia 解析并实时跟随。变量经 `@theme inline` 映射成 Tailwind 语义色(`bg-card`/`text-text-1` 等);**侧栏有独立 token 组**(`--sidebar-*`,light 模式也是深靛蓝侧栏,todo-flow 标志性外观)。glass 主题自动启用原生亚克力(`window.rs::set_acrylic`)+ 卡片 backdrop-blur。旧 102 套主题键启动时自动迁移(同名→同名,含 dark→dark,其余→light)。
