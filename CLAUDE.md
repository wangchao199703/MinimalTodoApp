...
**主题**(`src/lib/themes.ts` + `index.css`):玻璃拟态家族 4 色(glass 紫罗兰 / glass-ocean / glass-forest / glass-sunset)+ 经典 light / dark。玻璃系共用 `.glassy` 面板体系(白色半透明 + backdrop-blur),各变体 class 只覆盖强调色;渐变底在 `App.tsx` 的 `ThemeBackDrop`(BACKDROPS 表,135° 渐变 + 同色径向光晕)。light 为 `:root` 基线,`.dark` 经典深色。变量经 `@theme inline` 映射成 Tailwind 语义色(`bg-card`/`text-text-1` 等);**侧栏有独立 token 组**(`--sidebar-*`)。新增玻璃变体三步:VALID_THEMES + index.css 变体块(强调色)+ BACKDROPS 渐变。旧主题键启动时自动迁移(light/dark 保留,其余→glass)。
