...
1. **用户说完就记,记录用户提示词**:用户每说完一条需求,把**原始提示词**追加到根目录 `prompts.md`(不存在则创建,存在则续写,**剔除敏感信息**——密钥、token、隐私等),其余原文保留。**若被要求读某 md 文档:第一次读时把其内容记录下来,之后反复读时内容没变就不再重复记录。**
2. **做完即记,改动记录到 `release.md`**:每次改动后,把本次修改按**当前版本号**记到根目录 `release.md`(不存在则创建,存在则续写,剔除敏感信息——密钥、token、隐私等);记录时带上版本号,平时可详细写,正式 release 时再根据文档汇总精简。**用户没说改版本号,就绝不擅自提升版本号**;用户没说版本号,那就是上次的版本号,保持不变。
3. **做完即 commit,但不 push**:每轮做完就本地提交(中文消息,结尾带 Co-Authored-By);**等用户通知才 push / release**。
4. **做完即写交接文档**:每轮做完更新根目录 `handoff.md`(不存在则创建,存在则续写,剔除敏感信息——密钥、token、隐私等),讲清背景/进度/下一步,随时准备让其他人接手继续工作。
5. **做完就 run 起来让用户验证**:无特殊说明时,每轮做完把应用跑起来(`npm run tauri dev`),交用户走查。

## 工作风格(必守)

6. **不要假设用户清楚自己想要什么**:动机或目标不清晰时,停下来讨论;**如果用户提的需求有疑问点,停下来问用户,不要自己做决定。**
7. **目标清晰但路径不是最短的**:直接告诉用户,并建议更好的办法。
