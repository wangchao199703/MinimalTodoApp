...
> 最后更新:2026-06-23(v2.0.4 已发布)。

---

## 最新一轮:v2.0.4(2026-06-23,已发布)

详见 `release.md` v2.0.4 段与 `prompts.md` v2.0.4 批次。本轮要点(均已本地提交并发布):

- 托盘左键单击=召唤(不居中)、右键「显示并居中」区分;版本三处升 2.0.4。
- 新建待办优先级改上拉框;5 个新建弹框加标题(新 `ui/PopoverTitle.tsx`)。
- 截止/提醒上拉框三段分组 + 截止分钟 5 步长/精准开关 + 提醒实时预览/胶囊输入。
- 贴边窗口编辑时不自动收起(`DockState.hold` + `set_dock_hold` + `Popover` 计数)。
- 便签/分组重命名(双击 + 右键);便签标题同步修复(正文保存不带 custom_title)。
- 侧栏计数:已完成/四象限补数字;全部/标签/四象限/已完成统一只数父任务。
- 剪贴板图片预览根治(启动动态 `allow_directory` 把真实数据目录加入 asset scope + 灯箱 onError 回退)。
- **双击剪贴项自动粘贴到原光标处**(新 `paste.rs`,Ditto 同款,裸 FFI 零依赖):后台追踪外部前台窗口+焦点控件,写剪贴板→还原焦点→模拟粘贴键。设置「双击剪贴项自动粘贴」(默认开)+「粘贴按键」Ctrl+V/Shift+Insert。**待办边界**:管理员窗口(目标完整性级别更高)被 UIPI 拦时只弹通知提示手动粘贴——**未做提权 helper**(会破坏便携单 exe 模型),如需全自动粘进管理员窗口要另开一轮做 UIAccess/提权进程。
...
## 重新安装下载后装不上 — 运行中 exe 被锁,改就地替换 (v2.0.0)

**实测结论(本机)**:下载 OK(reqwest 异步 200、字节完整);卡在安装写盘——旧 `swap_and_restart` 把新版写到 `<exe目录>/MinimalTodoApp-v2.0.0-win-x64.exe`,当用户运行的就是同名已发布 exe 时,该路径=正在运行的 exe。覆盖/删除运行中 exe 失败(os 32 / os 5),但改名可行。

**修复**(`src-tauri/src/updater.rs` `swap_and_restart`):就地替换——① 运行中的 exe 改名挪开(`.old-<pid>`)② 原路径写新版 ③ bat 等旧进程退出→删旧(重试)→启原路径新版→自删。同路径同名、与资产名无关;写失败回滚改名;目录不可写则兜底写 `%LOCALAPPDATA%` 并 `--updated-from` 回收。`target_dir()` 已删(不再用)。
