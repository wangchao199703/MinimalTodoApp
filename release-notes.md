...
- **Theme favorites** — right-click any theme to add it to the **Favorites** group, now shown **first** (above Recent). Favorites can be **drag-reordered** and the order is saved; the group is never empty — it defaults to your current theme.
...
- **主题收藏** —— 右键任意主题即可加入「收藏」分组，现**置于最前**（在「常用」之上）。收藏内的主题可**拖动排序**且顺序会被保存；分组不会为空——默认收藏当前使用的主题。
